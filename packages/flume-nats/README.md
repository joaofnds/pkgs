...
- **Delivery count** comes from JetStream's `redeliveryCount` (1 on the first delivery,
  broker-tracked on redelivery); `nack()` maps to `nak()` and `ack()` to a
  server-confirmed `ackAck()`. `max_deliver` is unlimited — the core `Worker` owns the
  dead-letter decision and acks to stop redelivery.
